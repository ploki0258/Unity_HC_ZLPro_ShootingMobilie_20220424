...
namespace JACK
{
    public class AdsManager : MonoBehaviour, IUnityAdsInitializationListener, IUnityAdsLoadListener,IUnityAdsShowListener
    {
        ...
        private string gameIdAndroid = "4754881"; //後台 Android ID
        private string gameIdIos = "4754880"; //後台 ios ID
        ...
        {
            print("<color=red>2. 廣告初始化失敗，原因:" + message + "</color>");
        }
        ...
        private void Awake()
        {
            ...
            btnAdsAddCoin = GameObject.Find("廣告按扭添加金幣").GetComponent<Button>();
            btnAdsAddCoin.onClick.AddListener(LoadAds);

            InitializeAds();

            //#if程式區塊判斷式，條件達成才會執行該區塊
